...
# ── NEW: Dedicated Recommendation Route ──────────────────────────────────────
@app.route("/api/recommend", methods=["POST"])
def recommend():
    data = request.get_json()
    income = data.get("income_segment", "Medium")
    recent_list = data.get("recent_purchases", [])
    preferred = data.get("preferred_category", recent_list[0] if recent_list else "Electronics")
    age = int(data.get("age", 30))
    total_purchases = int(data.get("total_purchases", 10))
    ltv_est = min(100, round(total_purchases * 1.5 + (20 if income=="High" else 10 if income=="Medium" else 0), 1))

    recs = []
    seen_cats = set()

    for cat in recent_list[:3]:
        if cat in CATEGORY_RECS and cat not in seen_cats:
            seen_cats.add(cat)
            picks = np.random.choice(CATEGORY_RECS[cat], size=min(2,len(CATEGORY_RECS[cat])), replace=False)
            for prod in picks:
                recs.append({"product":prod,"category":cat,"reason":f"You recently bought from {cat}","confidence":round(min(0.97,0.88+ltv_est/1000),2),"basis":"Recent Purchase"})

    if income=="High":
        for cat in ["Electronics","Home & Kitchen"]:
            if cat not in seen_cats and len(recs)<8:
                seen_cats.add(cat); prod=np.random.choice(CATEGORY_RECS[cat])
                recs.append({"product":prod,"category":cat,"reason":"Premium pick based on your income","confidence":0.82,"basis":"Income: High"})
    elif income=="Medium":
        for cat in ["Clothing","Books"]:
            if cat not in seen_cats and len(recs)<8:
                seen_cats.add(cat); prod=np.random.choice(CATEGORY_RECS[cat])
                recs.append({"product":prod,"category":cat,"reason":"Best value for your budget","confidence":0.74,"basis":"Income: Medium"})
    else:
        for cat in ["Groceries","Toys"]:
            if cat not in seen_cats and len(recs)<8:
                seen_cats.add(cat); prod=np.random.choice(CATEGORY_RECS[cat])
                recs.append({"product":prod,"category":cat,"reason":"Budget-friendly recommendation","confidence":0.68,"basis":"Income: Low"})

    if age<30 and "Sports" not in seen_cats and len(recs)<8:
        seen_cats.add("Sports"); prod=np.random.choice(CATEGORY_RECS["Sports"])
        recs.append({"product":prod,"category":"Sports","reason":"Popular among customers your age","confidence":0.71,"basis":"Age Group"})
    elif age>=40 and "Books" not in seen_cats and len(recs)<8:
        seen_cats.add("Books"); prod=np.random.choice(CATEGORY_RECS["Books"])
        recs.append({"product":prod,"category":"Books","reason":"Popular among customers your age","confidence":0.69,"basis":"Age Group"})

    if preferred not in seen_cats and len(recs)<8:
        prod=np.random.choice(CATEGORY_RECS.get(preferred,["General Item"]))
        recs.append({"product":prod,"category":preferred,"reason":"Your preferred shopping category","confidence":0.79,"basis":"Preferred Category"})

    return jsonify({
        "recommendations": recs[:8],
        "input_summary": {"income_segment":income,"recent_purchases":recent_list,"preferred_category":preferred,"age":age,"total_purchases":total_purchases,"estimated_ltv":ltv_est}
    })
...
def ensure_data():
    """Generate CSV if not present (Render resets filesystem)"""
    if not os.path.exists("customers.csv"):
        print("Generating dataset...")
        import runpy
        runpy.run_path("generate_data.py")

# Auto-generate data + train on startup
ensure_data()
load_and_train()
